...
threshold_v = 0.0001            # threshold to detect forward swims from the scaled estimate
threshold_h = 0.0001            # threshold to detect turns from the scaled estimate
maxv = 0.20                     # maximum value allowed for forward velocity
maxh = 60.                      # maximum value allowed for heading change (convert)
logistic_filter_midpoint = 25.  # midpoint of the logistic weight function for inhibiting velocity based on heading
logistic_filter_steepness = 12. # steepness of the logistic weight function for inhibiting velocity based on heading
...
tail_tracking_step_size = 45    # step size between successive tail tracking points
...
n_caudal_points = 3             # number of caudal tail points (from the end of tail) to use for velocity estimation
...
tracker = TailTracker(framerate=maxfps, start_point=start_point, nsteps=tail_tracking_nsteps, step_size=tail_tracking_step_size, theta_range=theta_range,
...
while True:
    ...
    if ret:
        ...
        if broadcast_udp:
            message = struct.pack('>dd?i', velocity, theta, tracker.swimming, framecount)
